...
export function EditorPage() {
  ...
  const draggedCharacterRef = useRef<{ id: string; pointerId: number; startX: number; startY: number; x: number; height: number; maxX: number } | null>(null);
  const dragFrameRef = useRef<number | null>(null);
  const [draggedCharacterId, setDraggedCharacterId] = useState<string | null>(null);

  useEffect(() => {
    const move = (event: PointerEvent) => {
      const drag = draggedCharacterRef.current;
      const bounds = previewRef.current?.getBoundingClientRect();
      if (!drag || !bounds || event.pointerId !== drag.pointerId) return;
      const x = Math.max(-drag.maxX, Math.min(drag.maxX, drag.x + (event.clientX - drag.startX) / bounds.width * 2));
      const height = Math.max(0, Math.min(1, drag.height - (event.clientY - drag.startY) / bounds.height / 0.24));
      if (dragFrameRef.current !== null) cancelAnimationFrame(dragFrameRef.current);
      dragFrameRef.current = requestAnimationFrame(() => {
        const state = useStore.getState();
        state.patch({ characters: state.characters.map((character) => character.id === drag.id ? { ...character, x, height } : character) });
        dragFrameRef.current = null;
      });
    };
    const end = (event: PointerEvent) => {
      if (draggedCharacterRef.current?.pointerId !== event.pointerId) return;
  ...
  const startCharacterDrag = useCallback((event: React.PointerEvent<HTMLDivElement>, id: string) => {
    ...
    const character = useStore.getState().characters.find((item) => item.id === id);
    if (!character) return;
    const depthScale = 0.72 + ((character.depth + 1) / 2) * 0.56;
    const maxX = Math.max(0, 1 - (52 * character.scale * depthScale) / 100);
    draggedCharacterRef.current = { id, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, x: Math.max(-maxX, Math.min(maxX, character.x)), height: character.height, maxX };
  ...
  return (
    <div className="editor">
      <div
        ref={previewRef}
        className={`preview${draggedCharacterId ? ' is-dragging-character' : ''}`}
        onDragOver={(e) => {
          e.preventDefault();
          setDragging(true);
        }}
      ...
      >
        ...
        {s.characterMode === 'image' && <div className="stage-floor" aria-hidden="true" />}
