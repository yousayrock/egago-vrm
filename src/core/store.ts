...
import { DEFAULT_CHARACTER_ASSET_ID } from './characterAssets';
...
const DEFAULT_STAGE_CHARACTERS: StageCharacter[] = [
  { id: 'eriru-1', name: 'エリルたそ 1', spriteId: DEFAULT_CHARACTER_ASSET_ID, x: 0, height: 0, depth: 0, scale: 1, rotation: 0, facing: 1 },
];
      ...
      migrate: (persisted) => {
        ...
        return {
          ...
          characters: Array.isArray(state.characters) && state.characters.length
            ? state.characters.map((character) => ({ ...character, depth: character.depth ?? 0, rotation: character.rotation ?? 0, facing: character.facing ?? 1, spriteId: character.spriteId ?? DEFAULT_CHARACTER_ASSET_ID }))
